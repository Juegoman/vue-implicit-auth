  ...
  Object.defineProperties(this, {
    token: {
      ...
      set: function set(newToken) {
        // nonce check
        var decoded = decode(newToken.split('.')[1]);
        if (decoded.nonce !== _this.nonce) {
          throw new Error('Token nonce does not match stored nonce');
        }
        // logged in
        window.localStorage.setItem(_LSKEYS2.default.TOKEN, newToken);
        ...
        if (_this.authContext.reactiveChange) {
          _this.authContext.reactiveChange('token', newToken);
    ...
    decodedToken: {
      get: function get() {
        var splitToken = _this.token.split('.');
        return {
...
AADDriver.prototype.init = function (authContext) {
  ...
  if (returned) {
    ...
    var token = window.localStorage.getItem(_LSKEYS2.default.TOKEN);
    if (token) {
...
AADDriver.prototype.logout = function () {
  window.localStorage.removeItem(_LSKEYS2.default.TOKEN);
